...
export default function Search() {
  ...
  return (
    <div className="mx-auto max-w-6xl p-4 pb-24 sm:p-6">
      <PageHeader title="Search" subtitle="Documents, and vault items once matched" />
      ...
      <div className="mt-4 flex gap-2">
        ...
        />
        {isMobile && (
          <Button variant="secondary" onClick={() => setFiltersOpen(true)}>Filters</Button>
        )}
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 gap-6 lg:grid-cols-[220px_1fr]">
        ...
        <div>
          {!hasQuery && (
            <EmptyState title="Search your vault" description="Type to search documents by title, tag, or notes — or use a filter." />
          )}
          ...
          {hasQuery && !isLoading && documents.length === 0 && items.length === 0 && (
            <EmptyState title="No results" description="Try a different search term or filter." />
          )}

          {items.length > 0 && (
            <div className="mb-6">
              <p className="mb-2 text-sm font-medium text-neutral-500 dark:text-neutral-400">Vault items</p>
          ...
          {documents.length > 0 && (
            <>
              <p className="mb-2 text-sm font-medium text-neutral-500 dark:text-neutral-400">Documents</p>
              <div className="grid grid-cols-2 gap-4 sm:grid-cols-3 md:grid-cols-4">
                {documents.map((doc) => (
              ...
              {totalPages > 1 && (
                <div className="mt-6 flex items-center justify-center gap-3">
                  ...
                  <Button variant="secondary" size="sm" disabled={page >= totalPages || isFetching} onClick={() => setPage((p) => p + 1)}>Next</Button>
                </div>
              )}
            </>
          )}
      ...
      <Drawer isOpen={filtersOpen} onClose={() => setFiltersOpen(false)} side="bottom" size="lg" title="Filters">
        {filterPanel}
      </Drawer>
    </div>
  );
