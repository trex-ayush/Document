    ...
    >
          ...
          {NAV_ITEMS.map((item) => (
            <li key={item.to}>
              ...
              >
                ...
                {!isCollapsed && <span className="ml-2.5 font-medium whitespace-nowrap">{item.label}</span>}
              </NavLink>
            </li>
          ))}
        </ul>
      ...
      <div className="p-3 border-t border-neutral-100 dark:border-neutral-700">
        ...
        >
          ...
          {!isCollapsed && <span className="ml-2.5 text-sm font-medium">Collapse</span>}
        </button>
      </div>
    </aside>
  );
...
export function SidebarBrand({ collapsed }) {
  return (
    <div className="flex items-center gap-2.5 px-3 py-3">
      ...
      {!collapsed && <span className="text-sm font-semibold text-neutral-900 dark:text-neutral-100">Family Vault</span>}
    </div>
  );
}
