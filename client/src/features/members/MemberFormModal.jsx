...
const ACCESS_OPTIONS = [
  { value: 'read', label: 'Read only — can view & download' },
  { value: 'write', label: 'Read & write — can also add/edit/delete' },
];
    ...
    try {
      if (isEdit) {
        ...
        if (needsTempPassword && !data.tempPassword) {
          setError('tempPassword', { message: 'Set a temporary password, or turn on "Send invite email"' });
          return;
        }
        const payload = {
          name: data.name,
        ...
        toast.success(effectiveSendInvite ? 'Invite sent' : 'Member added');
      }
      onSaved?.();
      onClose?.();
    } catch (err) {
      ...
      if (code === 'EMAIL_TAKEN') {
        ...
        toast.error(err?.response?.data?.message || 'Could not save this member.');
      }
    }
  };
  ...
  return (
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} noValidate className="space-y-4">
        <Input label="Name" placeholder="e.g. Priya Singh" error={errors.name?.message} {...register('name', { required: 'Name is required' })} />
        <Input label="Relation" placeholder="e.g. Spouse, Child, Parent" {...register('relation')} />
        ...
        {isEdit ? (
          ...
          <>
            <Switch
              ...
              description="Off creates a profile-only record (e.g. a child or pet) with no login."
              checked={canLogin}
            ...
            {canLogin && (
              <div className="space-y-4 rounded-lg border border-neutral-200 dark:border-neutral-700 p-3">
                <Input label="Email" type="email" placeholder="them@example.com" error={errors.email?.message} {...register('email')} />
                ...
                <Switch
                  ...
                  description={
                    emailEnabled
                      ? 'They set their own password by accepting the emailed invite.'
                      : 'Email is not configured for this family — set a temporary password below instead.'
                  }
