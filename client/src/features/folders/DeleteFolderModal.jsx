...
export default function DeleteFolderModal({ isOpen, onClose, folder, onDeleted }) {
  ...
  const handleConfirm = async () => {
    try {
      ...
      toast.error(err?.response?.data?.message || 'Could not delete the folder');
    }
  };
  ...
  return (
    ...
    >
      {checking ? (
        ...
        <p className="text-sm text-neutral-600 dark:text-neutral-400">This folder is empty. This can't be undone.</p>
      ) : (
        <div className="space-y-2 text-sm text-neutral-600 dark:text-neutral-400">
          <p className="font-medium text-red-600 dark:text-red-400">
            This will permanently delete everything inside this folder:
          </p>
          <ul className="list-disc space-y-1 pl-5">
            {counts?.folderCount > 0 && <li>{counts.folderCount} subfolder{counts.folderCount === 1 ? '' : 's'}</li>}
            {counts?.documentCount > 0 && <li>{counts.documentCount} document{counts.documentCount === 1 ? '' : 's'}</li>}
            {counts?.fileCount > 0 && <li>{counts.fileCount} file{counts.fileCount === 1 ? '' : 's'}</li>}
          </ul>
