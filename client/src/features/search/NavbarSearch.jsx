        ...
        <span className="absolute right-2 top-1/2 flex -translate-y-1/2 items-center gap-1">
          {hasText ? (
            <button
              ...
              className="flex h-7 w-7 items-center justify-center rounded-md text-neutral-400 hover:bg-neutral-100 hover:text-neutral-600 dark:hover:bg-neutral-700 dark:hover:text-neutral-200"
            >
      ...
      {showDropdown && (
        ...
        >
          ...
          <button
            ...
            className="flex w-full items-center justify-between gap-2 border-t border-neutral-100 px-4 py-2.5 text-left text-sm font-medium text-primary-600 hover:bg-neutral-50 dark:border-neutral-700 dark:text-primary-400 dark:hover:bg-neutral-700/60"
          >
            <span className="truncate">{t('navbar.seeAll', 'See all results for “{{query}}”', { query: value.trim() })}</span>
            <span className="hidden flex-shrink-0 text-xs font-normal text-neutral-400 sm:inline">{t('navbar.enterHint', 'Enter')}</span>
          </button>
        </div>
      )}
    </div>
