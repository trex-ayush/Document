...
describe('document-types CRUD', () => {
  it('GET lists the 11 seeded defaults', async () => {
    ...
    const res = await request(app).get('/api/document-types').set('Authorization', `Bearer ${s.accessToken}`);
  ...
  it('admin can create, patch and delete a custom type', async () => {
    ...
    const list = await request(app).get('/api/document-types').set('Authorization', `Bearer ${s.accessToken}`);
